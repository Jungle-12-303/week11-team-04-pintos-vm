 ...
 * failed. */
/* 사용자 가상 페이지 UPAGE를 커널 가상 주소 KPAGE로 식별되는 물리 프레임에 매핑하여 페이지 맵 레벨 4(PML4)에 추가합니다.
 * UPAGE는 이미 매핑되어 있어서는 안 됩니다. KPAGE는 palloc_get_page()를 통해 사용자 풀에서 확보한 페이지여야 합니다.
 * WRITABLE이 true인 경우, 새 페이지는 읽기/쓰기 가능하고;
 * 그렇지 않은 경우 읽기 전용입니다.
